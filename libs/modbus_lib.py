...
class ModbusClient:
    ...
    def read_register_FOR_TAOSONIC_ONLY(self, start_address, register_count, data_format='>f'):
...
class DeviceManager:
    ...
    def read_register_FOR_TAOSONIC_ONLY(self, device_id, start_address, register_count, data_format):
        function_code = 0x03
        ...
        # self.print_bytes_info("swapped_data", swapped_data)
        try:
            floating_point = struct.unpack(data_format, swapped_data)[0]
        except struct.error:
            print(f'Error decoding data from device {device_id}')
