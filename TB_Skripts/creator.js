...
function AddEntityDialogController(instance) {
    ...
    vm.zgm_system_types = ["ZGM-D-P-924", "ZGM-D-PF-924"];

    vm.addEntityFormGroup = vm.fb.group({
     zgm_system_type: [null, [vm.validators.required]],
    });
    ...
    vm.filteredZGMSystemTypes = vm.zgm_system_types;
    vm.filterZGMSystemTypes=function(value) {
        if(value!=undefined){
            const filterValue = value.toLowerCase();
            vm.filteredZGMSystemTypes = vm.zgm_system_types.filter(type => type.toLowerCase().includes(filterValue));
        }
        else{
            vm.filteredZGMSystemTypes = vm.zgm_system_types;
        }
...
// return
  ...
  "keyFilters": [
	{"key":{"type":"ENTITY_FIELD","key":"name"},"valueType":"STRING","predicate":{"operation":"STARTS_WITH","value":{"defaultValue":vm.addEntityFormGroup.value.zgm_system_type,"dynamicValue":null},"type":"STRING"}}  ],
  "latestValues": [],
  "pageLink": {
    "dynamic": true,
    "page": 0,
    ...
    "textSearch": ""
            ...
            function (result) {
                ...
                if(result[0].data.length==0){
                    _assetName=vm.addEntityFormGroup.value.zgm_system_type+"_0001"
                }
                else{
                    var nameArray=result[0].data[0].latest.ENTITY_FIELD.name.value.split("_")
                    var lastIndex=parseInt(nameArray[nameArray.length - 1]);
                    _assetName=vm.addEntityFormGroup.value.zgm_system_type+"_"+intToFourDigitString(lastIndex+1)
                }
            ...
            widgetContext.rxjs.forkJoin([assetService.saveAsset({name: _assetName,type: "ZGM-D-924",label: _assetName}),
            ...
            widgetContext.entityService.edgeService.saveEdge({name: _edgeName,type: "ZGM-D-924XXXX-EDGE",label: _edgeName
                ,routingKey: guid()
