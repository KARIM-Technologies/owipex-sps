...
CHECK_INTERVAL = 10
STATE_FILE = '/etc/owipex/state_file.txt'  # Pfad zur Zustandsdatei
...
def write_state(manually_stopped):
    with open(STATE_FILE, 'w') as f:
        f.write('stopped' if manually_stopped else 'running')
...
def read_state():
    try:
        ...
        return False  # Zustandsdatei nicht gefunden, gehe von nicht gestoppt aus
...
def monitor_system():
    ...
    while True:
        ...
        manually_stopped = read_state()
        if main_process is not None and main_process.poll() is None:
            set_led_color('G')
            print("Hauptskript läuft, LED Grün.")
        elif not manually_stopped:
            print("Hauptskript unerwartet gestoppt oder bei Boot nicht laufend, versuche zu starten.")
            start_main_script()
            ...
            set_led_color('B')
            print("Server erreichbar, Hauptskript nicht gestartet oder manuell gestoppt, LED Blau.")
